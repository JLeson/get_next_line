...
/*   Updated: 2023/01/06 16:44:05 by fsarkoh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buff)
		return (NULL);
	tmp_buff = get_line_buff(fd, buff);
	if (!tmp_buff)
		return (free_and_return(&buff, NULL));
	line = get_line_from_buff(tmp_buff);
	if (!line)
		return (free_and_return(&buff, &tmp_buff));
	free(buff);
	buff = rm_line_from_buff(tmp_buff);
	free(tmp_buff);
	if (*line == '\0')
		return (free_and_return(&buff, &line));
	return (line);
}
